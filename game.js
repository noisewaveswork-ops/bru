...
class Bullet {
    constructor(x, y, angle, speed, isEnemy = true, image = null) {
        this.x = x;
        this.y = y;
        this.angle = angle;
        this.speed = speed;
...
class HomingBullet extends Bullet {
    constructor(x, y, game) {
        super(x, y, -Math.PI / 2, 6, false, game.homingBulletImg);
        this.game = game;
        this.width = 14;
        this.height = 6;
        this.damage = 0.4;
...
class Enemy {
    constructor(x, y, pattern, image) {
        this.x = x;
        this.y = y;
        this.pattern = pattern;
        this.image = image; // кастомный спрайт
        this.timer = 0;
...
class Boss {
    constructor(x, y, game) {
        ...
        this.xDirection = 1;
        this.xSpeed = 1.5;
        this.image = game.bossImg; // кастомный спрайт
    }
    ...
    update() {
        ...
        // Паттерны фаз с использованием кастомных спрайтов для пуль
        const ebImg = this.game.enemyBulletImg;

        if (this.phase === 1) {
            if (this.timer % 16 === 0) {
                for (let i = 0; i < 6; i++) {
                    const spread = (i - 2.5) * 0.20;
                    const angle = Math.PI / 2 + spread + Math.sin((this.timer + i * 10) * 0.06) * 0.22;
                    const bullet = new Bullet(this.x, this.y + 10, angle, 2.0, true, ebImg);
        ...
        if (this.phase === 2) {
            if (this.timer % 150 === 0) {
                ...
                for (let i = 0; i < rows; i++) {
                    const y = 140 + i * 70;
                    const left = new Bullet(-30, y, 0, 1.1, true, ebImg);
                    left.width = 36;
                    left.height = 36;
                    const right = new Bullet(430, y + 35, Math.PI, 1.1, true, ebImg);
                    right.width = 36;
                    right.height = 36;
                    this.game.bullets.push(left, right);
                }
                this.game.sound.enemyShoot();
            }
            if (this.timer % 90 === 0) {
                ...
                for (let i = -1; i <= 1; i++) {
                    this.game.bullets.push(new Bullet(this.x, this.y, angle + i * 0.18, 2.8, true, ebImg));
                }
            }
        }
        ...
        if (this.phase === 3) {
            if (this.timer % 5 === 0) {
                ...
                for (let i = 0; i < 2; i++) {
                    ...
                    const bullet = new Bullet(this.x, this.y, angle, 3.2, true, ebImg);
                    bullet.width = 7;
                    bullet.height = 7;
                    this.game.bullets.push(bullet);
                }
                if (this.timer % 9 === 0) {
                    for (let i = 0; i < 2; i++) {
                        const angle = -base * 0.8 + Math.PI * i;
                        const bullet = new Bullet(this.x, this.y, angle, 2.4, true, ebImg);
                        bullet.width = 10;
                        bullet.height = 10;
                        this.game.bullets.push(bullet);
                    }
    ...
    draw(ctx) {
        ...
        // Полоска здоровья (всегда)
        const bw = 180, bh = 8, bx = 110, by = 45;
        ctx.fillStyle = '#111';
        ctx.shadowBlur = 0;
...
class Game {
    constructor() {
        ...
        this.bombImg.src = 'assets/bomb.png';

        this.playerBulletImg = new Image();
        this.playerBulletImg.src = 'assets/bullet.png';
        this.enemyBulletImg = new Image();
        this.enemyBulletImg.src = 'assets/bullet_enemy.png';
        this.homingBulletImg = new Image();
        this.homingBulletImg.src = 'assets/bullet_homing.png';

        this.bossImg = new Image();
        this.bossImg.src = 'assets/enemy_boss.png';

        // Спрайты врагов по типам
        this.enemyImages = {
            straightShooter: new Image(),
            sineFan: new Image(),
            spiral: new Image(),
            sideSweeper: new Image()
        };
        this.enemyImages.straightShooter.src = 'assets/enemy_straight.png';
        this.enemyImages.sineFan.src = 'assets/enemy_sine.png';
        this.enemyImages.spiral.src = 'assets/enemy_spiral.png';
        this.enemyImages.sideSweeper.src = 'assets/enemy_sweeper.png';
    ...
    defineWavePatterns() {
        // В паттернах теперь используется кастомный enemyBulletImg (доступен через this)
        const self = this; // для стрелочных функций (или просто this)
        this.patterns = {
            straightShooter: {
                health: 2, points: 100,
                update: (enemy) => {
                    ...
                    if (enemy.y < 450 && enemy.timer % 75 === 0) {
                        const angle = Math.atan2(self.player.y - enemy.y, self.player.x - enemy.x);
                        self.bullets.push(new Bullet(enemy.x, enemy.y, angle, 2.5, true, self.enemyBulletImg));
                        self.sound.enemyShoot();
                    }
                }
            },
            sineFan: {
                ...
                update: (enemy) => {
                    ...
                    if (enemy.y < 450 && enemy.timer % 65 === 0) {
                        const baseAngle = Math.atan2(self.player.y - enemy.y, self.player.x - enemy.x);
                        for (let i = -1; i <= 1; i++) {
                            self.bullets.push(new Bullet(enemy.x, enemy.y, baseAngle + i * 0.3, 3, true, self.enemyBulletImg));
                        }
                        self.sound.enemyShoot();
                    }
                }
            ...
            spiral: {
                ...
                update: (enemy) => {
                    ...
                    if (enemy.y < 420 && enemy.timer % 7 === 0) {
                        ...
                        for (let i = 0; i < 2; i++) {
                            const angle = base + Math.PI * i;
                            const bullet = new Bullet(enemy.x, enemy.y, angle, 2.1, true, self.enemyBulletImg);
                            bullet.width = 8;
                            bullet.height = 8;
                ...
                update: (enemy) => {
                    ...
                    if (enemy.y < 450 && enemy.timer % 60 === 0) {
                        ...
                        for (let j = 0; j < 2; j++) {
                            setTimeout(() => {
                                if (enemy.health > 0) {
                                    self.bullets.push(new Bullet(enemy.x, enemy.y, angle, 3.5, true, self.enemyBulletImg));
                                    self.sound.enemyShoot();
                                }
                            }, j * 120);
                        }
    ...
    // buildWave – без изменений (используются те же ключи)
    buildWave(waveNumber) {
        const queue = [];
        if (waveNumber === 1) {
            for (let i = 0; i < 5; i++) {
                queue.push({ type: 'straightShooter', x: 70 + i * 65, y: -40, delay: i * 45 });
            }
        } else if (waveNumber === 2) {
            for (let i = 0; i < 4; i++) {
                queue.push({ type: 'sineFan', x: 80 + i * 80, y: -50, delay: i * 55 });
            }
        } else if (waveNumber === 3) {
            queue.push({ type: 'sideSweeper', x: -20, y: 120, delay: 0 });
            queue.push({ type: 'sideSweeper', x: 420, y: 180, delay: 60 });
            queue.push({ type: 'straightShooter', x: 200, y: -50, delay: 120 });
        } else if (waveNumber === 4) {
            queue.push({ type: 'spiral', x: 130, y: -40, delay: 0 });
            queue.push({ type: 'spiral', x: 270, y: -40, delay: 80 });
            queue.push({ type: 'sineFan', x: 200, y: -60, delay: 150 });
        } else if (waveNumber === 5) {
            for (let i = 0; i < 6; i++) {
                queue.push({ type: 'straightShooter', x: 40 + i * 60, y: -30, delay: i * 18 });
            }
            queue.push({ type: 'spiral', x: 200, y: -50, delay: 120 });
        } else if (waveNumber === 6) {
            queue.push({ type: 'sideSweeper', x: -20, y: 120, delay: 0 });
            queue.push({ type: 'sideSweeper', x: 420, y: 120, delay: 40 });
            queue.push({ type: 'spiral', x: 200, y: -40, delay: 100 });
            queue.push({ type: 'sineFan', x: 90, y: -40, delay: 170 });
            queue.push({ type: 'sineFan', x: 310, y: -40, delay: 200 });
        } else if (waveNumber === 7) {
            for (let i = 0; i < 3; i++) {
                queue.push({ type: 'spiral', x: 80 + i * 120, y: -40, delay: i * 50 });
            }
            queue.push({ type: 'sideSweeper', x: -20, y: 200, delay: 180 });
            queue.push({ type: 'sideSweeper', x: 420, y: 240, delay: 220 });
            ...
            queue.push({ type: 'sideSweeper', x: -20, y: 160, delay: 150 });
            queue.push({ type: 'sideSweeper', x: 420, y: 160, delay: 190 });
        } else if (waveNumber === 9) {
            for (let i = 0; i < 4; i++) {
                queue.push({ type: 'spiral', x: 60 + i * 90, y: -40, delay: i * 35 });
            }
            queue.push({ type: 'sideSweeper', x: -20, y: 180, delay: 180 });
            queue.push({ type: 'sideSweeper', x: 420, y: 180, delay: 220 });
        }
        return queue;
    }
    ...
    spawnFromQueue() {
        ...
        while (this.waveSpawnQueue.length > 0 && this.spawnTimer >= this.waveSpawnQueue[0].delay) {
            ...
            if (pattern) {
                const enemy = new Enemy(spec.x, spec.y, pattern);
                // Назначаем кастомный спрайт врагу
                enemy.image = this.enemyImages[spec.type];
    ...
    update() {
        ...
        if (!this.laserMode) {
            if (now - this.player.lastShotTime > 120) {
                this.bullets.push(new Bullet(this.player.x, this.player.y - 15, -Math.PI / 2, 9, false, this.playerBulletImg));
    ...
    drawUI() {
        ...
        const UI = {
            ...
            wave: { x: 370, y: 70, size: 12, color: '#d9d9d9' },
            bombs: this.isMobile ? {
        ...
        // Жизни – только оставшиеся
        const lv = UI.lives;
        for (let i = 0; i < this.player.lives; i++) {
            const x = lv.x + i * (lv.size + lv.gap);
            const y = UI.panelY + lv.y;
            if (this.heartImg && this.heartImg.complete && this.heartImg.naturalWidth > 0) {
                ctx.drawImage(this.heartImg, x, y, lv.size, lv.size);
            } else {
                ctx.fillStyle = '#ff3366';
                ctx.beginPath();
                ctx.arc(x + lv.size/2, y + lv.size/2, lv.size/2, 0, Math.PI * 2);
                ctx.fill();
            }
        }
        ...
        if (this.isMobile) {
            for (let i = 0; i < this.player.bombs; i++) {
                ...
                if (this.bombImg && this.bombImg.complete && this.bombImg.naturalWidth > 0) {
                    ctx.drawImage(this.bombImg, x, y, bv.size, bv.size);
                } else {
    ...
    draw() {
        ...
        this.bullets.forEach(b => { if (!b.isEnemy) b.draw(this.ctx); });
